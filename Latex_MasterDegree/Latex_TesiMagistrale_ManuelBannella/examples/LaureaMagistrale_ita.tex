...
L'ambiente spaziale ed in particolare zone del cosmo come le fasce di "Van Allen" rappresentano tutt'ora una sfida per quanto riguarda problemi di radiazioni spaziale e conseguente problemi relativi alla consistenza della memoria dell'intero sistema. Le radiazioni cosmiche possono infatti generare corruzioni all'interno di ogni componente hardware ed in particolare nelle zone di memorie volatili.

In questo lavoro sono state usate diverse tecniche software con l'obiettivo di rilevare e correggere eventuali errori e malfunzionamenti del sistema hardware ed in particolare: - utilizzo di ridondanza software (di copie dello stesso firmware), - protezione dell'informazione tramite algoritmi di rilevazione e correzione dell'errore, - diagnostica ed eventuale esclusione dei sottosistemi non correttamente funzionanti.
...
ed inoltre ogni nuovo sviluppo ha superato una serie di test creati con l'obiettivo di valutare sia le prestazioni software, sia il rispetto dei requisiti richiesti per il tipo di applicazione richiesta.

I risultati ottenuti durante la fase di testing dei diversi applicativi hanno determinato un sensibile irrobustimento di tutto il firmware. Sebbene non sia stato possibile determinare sul microcontrollore i fattori come il consumo di memoria e di tempo (data l'impossibilità di simulare una situazione reale di volo), l'architettura software è stata resa il più possibile scalabile e quindi modificabile in base ai risultati di integrazioni del firmware con il sistema reale.
In generale, l'architettura di controllo delle corruzioni sul sistema a tenuto conto di un compromesso tra fattori quali l'occupazione di memoria, l'occupazione temporale della cpu e la protezione del sistema.
\end{abstract}
