...
the \LaTeXe\ class \texttt{sapthesis.cls} developed by Manuel Bannella
...
FourDimensionalParityCode.c:

\begin{lstlisting}[language=c][fontsize=\small]
#include "FourDimensionalParityCode.h"

void toggleBit(int16_t *ptr,int16_t len,int16_t i,int16_t j)
{
    if(i<len)
    {
        ptr[i] ^= 1UL << (j);  //  Check to write inside the matrix and not beyond
    }
}
...
int16_t Calc_HParity(int16_t Data[])
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        {
            if (Data[i] & index)
            { // Check the bit value at index position
                temp = temp ^ 0x0100;
            }
            index = index >> 1;
        }
...
int16_t Calc_VParity(int16_t Data[]){
    ...
    int16_t i;
    for(i=0; i<8; i++){
        parity = parity ^ Data[i];// XOR whole word/byte in single operation
    }
    return parity;
}
int16_t Calc_DParity(int16_t Data[])
{
    ...
    dparity = dparity << 8;     // Make the space for parity bits
    dparity = dparity ^ parity; // Concatenate the two parity bits
    return dparity;
}
int16_t find_position(int16_t a, int16_t *ptr)
{
...
int16_t count_errors(int16_t a)
{ // Simply calculates number of ones in ’a’
    int16_t i;
    int16_t index = 1;
    int16_t count = 0;
    for (i = 0; i < 16; i++)
...
\begin{figure}[htbp]
\centerline{\includegraphics[scale=1]{examples/scrubbingTestParziali.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[scale=0.47]{examples/corruptionCheckAsm.PNG}}
...
void sensorDiag(void)
{
    ...
    if(majorityDecisionByte(diagnosticFlags.magnetometerSensorFlag)==0x0)
    {
    ...
    if(majorityDecisionByte(diagnosticFlags.cpuTemperatureSensorFlag)==0x0)
    {
        counter+=checkTempCpuId(&diagnosticFlags.cpuTemperatureSensorFlag);
    }
    else
    {
        if(majorityDecisionByte(diagnosticFlags.cpuTemperatureSensorSecondFlag)==0x0)
        {
    ...
    if(majorityDecisionByte(diagnosticFlags.fpgaTemperatureSensorFlag)==0x0)
    {
        counter+=checkTempFpgaId(&diagnosticFlags.fpgaTemperatureSensorFlag);
    }
    else
    {
        if(majorityDecisionByte(diagnosticFlags.fpgaTemperatureSensorSecondFlag)==0x0)
        {
    ...
    if(majorityDecisionByte(diagnosticFlags.rtcFlag)==0x0)
    {
...
*/
    ...
    AB_ADDRESS_ACC, ADXL345DEVID) ) == 0x00)
    {
        counter++;
        if(counter > 30000)  
        {
            return AUTO_ACC_RETURN_VALUE+AUTO_I2C_RETURN_VALUE;
        /*
        *
        * Il valore dipende dalla priorità con cui bisogna risolvere 
        * il problema: questo identifica che sia il 
        * bus IC che l'accelerometro sono non funzionanti:
        *
    ...
    if(abacus_i2c_readRegister(AB_I2C_BUS00, AB_ADDRESS_ACC, 
    ADXL345CONFIGREG)!=ADXL345_STD_CONFIG)
    {
        //Calibra il sensore
...
*/
    ...
    while((result = abacus_i2c_readRegister(AB_I2C_BUS00, 
    AB_ADDRESS_GYRO2014, L3G4200_DEVID))==0x00)
    {
        counter++;
        if(counter > 30000)            
        {
            return AUTO_GYR_RETURN_VALUE+AUTO_I2C_RETURN_VALUE;
            /*
            * 0b1000 0000 + 0b0000 0010 = 0b1000 0010 = 130
            *  I2C fault     gyro FAULT
            *
            */
        }
    }
    ...
    diagnosticFlags.I2CFlag=0x00; 
    if(abacus_i2c_readRegister(AB_I2C_BUS00, AB_ADDRESS_GYRO2013, 
    L3G4200CONFIGREG)!=L3G4200_STD_CONFIG)
        {
            //Calibra il sensore
            abacus_sensors_gyro_init(AB_ABACUSVERSION2014);
        }
...
uint8_t checkTempCpuId(uint8_t* flag)
{
    ...
    *flag=0xFF;
    ...
    buffertmp = abacus_i2c_readRegister(AB_I2C_BUS00, 
    AB_ADDRESS_TEMPCPU, DS1775CONFIGREG);
...
*  Ricapitolando:
*  Ho due flag:
*  a) Flag_Diagnostic_Part.cpuTemperatureSensorFlag
*  b) Flag_Diagnostic_Part.cpuTemperatureSensorSecondFlag
*
*  Possibiliti combinazioni:
*
*  1) cpuTemperatureSensorFlag == 0 
*  && cpuTemperatureSensorSecondFlag == 0 tutto funziona bene
*  2) cpuTemperatureSensorFlag == 1 
*  && cpuTemperatureSensorSecondFlag == 0 prima volta errore
*  3) cpuTemperatureSensorFlag == 0 
*  && cpuTemperatureSensorSecondFlag == 1 NON SUCCEDE MAI
*  4) cpuTemperatureSensorFlag == 1 
*  && cpuTemperatureSensorSecondFlag == 1 devo disattivare
*/
        abacus_sensors_temperature_init(); 
        return AUTO_CPU_TEMP_RETURN_VALUE;
    }
...
/*
 *
 * Il valore dipende dalla priorità con cui bisogna risolvere il problema: 
 * questo identifica che sia il bus IC che l'accelerometro sono non funzionanti:
 *
 * 0b1000 0000 + 0b0010 0000 = 0b1010 0000 = 128+32 = 160
 *  I2C fault     FPGA temp FAULT
 *
...
*/
    ...
    buffertmp = abacus_i2c_readRegister(AB_I2C_BUS00, 
    AB_ADDRESS_TEMPFPGA, DS1775CONFIGREG);
    diagnosticFlags.I2CFlag=0x00;  // HO quindi abbassato la flag sia del sensore che del bus
...
*  Ricapitolando:
*  Ho due flag:
*  a) Flag_Diagnostic_Part.fpgaTemperatureSensorFlag
*
*  Possibiliti combinazioni:
*
*  1) fpgaTemperatureSensorFlag == 0 
*  && fpgaTemperatureSensorSecondFlag == 0 tutto funziona bene
*  2) fpgaTemperatureSensorFlag == 1 
*  && fpgaTemperatureSensorSecondFlag == 0 riscontro 
*  per la prima volta un problema
*  3) fpgaTemperatureSensorFlag == 0 
*  && fpgaTemperatureSensorSecondFlag == 1 NON SUCCEDE MAI
*  4) fpgaTemperatureSensorFlag == 1 
*  && fpgaTemperatureSensorSecondFlag == 1 devo disattivare 
*/
        abacus_sensors_temperature_init(); 
        return AUTO_FPGA_TEMP_RETURN_VALUE;
    }
}
...
int main(void)
{
    ...
    (int16_t)PERSISTENT_RAM_BYTE_SIZE) )
        ...
        else
        {
            ...
            backupDataInPersistentRam((uint16_t*)MEMORY_TO_BACKUP_STARTING_POINTER, 
            (uint8_t*)PERSISTENT_RAM_STARTING_POINTER, (uint32_t)PERSISTENT_RAM_BYTE_SIZE);
        }

    //Check if the space orbit is reached, otherwise we stop the start-up for 100 seconds
    uint8_t satelliteInSpace=isSatelliteInSpace();
   ...
   //Check flight status and boot into preprogrammed sequence
    ...
    while (1)
    {
        ...
        //No action  for 60 seconds, system go to sleep and wait until the new action is needed
